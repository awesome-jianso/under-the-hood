...
再看 Go 如何把成熟的事件机制藏进运行时，让开发者写同步代码、跑事件驱动的 I/O。
...
- **就绪模型**（`select`/`poll`、epoll、kqueue）：内核说「这个 fd 就绪了」，**由调用方**去做非阻塞 I/O。
- **完成模型**（Windows IOCP、Linux io_uring）：调用方说「把这段 I/O 做了，完成时通知我」，
  内核接管整个操作，调用方事先交出缓冲区、事后收割完成事件。
...
Go 的独到之处，是把 Reactor 藏在运行时底下，对用户只呈现同步阻塞的代码：开发者既不写回调，
