...
判断一次足矣，何必循环？答案是：`Wait` 返回时，那个让 goroutine 睡去的条件**不保证**仍然成立。
...
不过这条保证只覆盖「唤醒与被唤醒」这一对事件之间的可见性，它**不**保证条件本身成立，
那是 11.4.2 已经讲过的 Mesa 语义另说的事。可见性与条件成立是两码事，前者靠内存模型，
后者靠那个 `for` 循环。这也从内存模型一侧印证了为什么裸用 `if` 不安全。
...
才请出来的工具。它仍在标准库里，文档却反过来劝读者先想想 channel，这种「提供但不鼓励」的姿态，
本身就是 Go 并发取向的一个注脚。
