...
`len` 是能索引到的范围，`cap` 是「在不重新分配的前提下还能涨到多大」。两者分离正是切片能做
...
所以一个切片实际的 `cap` 常略大于心算的值。它是「乘性增长 + 尺寸类对齐」两步合成的结果：
前者保证摊还 $O(1)$，后者顺手把分配器本就要浪费的那点边角料还给读者。这解释了那个常见困惑,
...
**子切片导致的内存泄漏。** 切片头部只要活着，它指向的**整段**底层数组就回收不掉，哪怕只用得着
开头十个元素：
