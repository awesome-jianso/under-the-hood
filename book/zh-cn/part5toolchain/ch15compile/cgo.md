...
当读者在 Go 源文件里 `import "C"`，写下一行 `C.foo()` 时，就已经踏出了 Go 这门语言
...
编译器替读者铺好了桥头。读者写下 `C.foo()` 时，cgo 工具会生成一个 Go 包装函数 `_Cfunc_foo`，
...
（一次调用做很多事），而**极不适合**放进热点的、细粒度的循环里反复跨界。如果读者发现自己在一个
...
所以「cgo is not Go」不只是一句关于性能的提醒。它是说：一旦 `import "C"`，项目就不再
...
同步更新它。第二条则是第一条的递归版本：哪怕递过去的外层指针守规矩，只要它指向的内存里**藏着**
...
当确实需要一块**生命周期跨越调用、长期被 C 持有**的内存时，正确的做法不是和检查器对抗，
而是把内存放到 GC 管不着的地方：
...
`C.malloc` 把内存搬出 GC 的管辖，代价是要自己负责 `free`，Go 的内存安全在这块内存上不再
