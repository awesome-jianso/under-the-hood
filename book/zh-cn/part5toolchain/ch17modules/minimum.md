...
**能满足所有模块要求的最低版本**。先用本书一贯的小例子建立直觉。读者的程序直接要求 C ≥ v1.2;
...
的是「最小」并不意味着「陈旧」：当读者想要新版本，`go get` 一句话就升，工具会把「使用新版本」
这个答案明明白白地还给读者，由读者决定何时接受。
...
至于前沿，依赖管理仍有未尽的张力。MVS 给出可重现与可预测，但它**不**主动告知「读者依赖的某版本
