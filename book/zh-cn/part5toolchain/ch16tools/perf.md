...
优化的第一原则是先测量，再动手。凭直觉猜瓶颈往往猜错，真正的热点常常不在读者以为的地方。Go 把
...
`allocs/op` 这一列价值极高，它直接告诉读者「这段代码每跑一次在堆上分配了几个对象」。每一次堆分配
...
基准告诉读者「多快」，但要想知道「这点时间花在了哪些函数上」，就要用 `pprof`。它采集几类
...
与堆的轻量采样，于是把「是否值得这份开销」的决定权交还给读者，用 `SetBlockProfileRate` 与
...
flowchart TD
    ...
    PROFILE --> HOTSPOT["火焰图定位热点<br/>（往往不在预想的地方）"]
    HOTSPOT --> FIX["针对热点修改"]
