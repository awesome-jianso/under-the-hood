...
**竞态检测器**（`-race`）正是把这件事从「靠运气复现」变成「跑一遍就告诉读者」的工具。这一节
...
按官方文档给出的实测，这套机制使程序的**内存占用约增大 5 至 10 倍、运行时间约变慢 2 至 20
倍**；此外每个 `defer`/`recover` 会多占约 8 字节，长期运行且周期性 `defer`/`recover` 的
goroutine 要留意这点。
...
其二，它**有成本**。前一节那组 5 至 10 倍内存、2 至 20 倍变慢的数字，是插桩与影子内存的必然
开销。所以 `-race` 的定位是**测试与预发布**：让它进 CI、进压测、进集成环境，而不是常开在生产。把
...
不可依赖。这是一份规范，本身不会替读者检查任何程序。竞态检测器负责回答「程序错在哪」：它把
...
给出正确同步的手段，一边用竞态检测器抓出没同步好的地方。「并发代码必过 `-race` 测试」因此
是写可靠 Go 并发程序的基本功，内存模型定义对错，竞态检测器帮读者发现错。
