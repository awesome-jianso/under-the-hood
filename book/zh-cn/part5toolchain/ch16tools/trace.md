...
成一棵调用树，告诉读者 `json.Marshal` 占了 30% 的 CPU。但它答不了另一类问题：一个请求耗了
...
进出。把这条时间线交给 `go tool trace` 渲染，读者便能逐毫秒地看清程序里发生了什么。这一节讲它
...
大多数事件都附带一个**纳秒精度的时间戳**和一份**栈回溯**。这意味着 trace 不仅告诉读者「这里
发生了一次阻塞」，还告诉读者「是哪一行代码、在哪个调用栈深处阻塞的」。把这些事件按 P、按
goroutine 在时间轴上铺开，`go tool trace` 画出一张可交互的时间线，读者能直接读出：这一刻有
...
光有运行时事件，有时还不够定位**业务**层面的延迟，因为追踪器看不懂业务上的「一次请求」对应哪些
...
- **某个 goroutine 在某事件后停住、很久才被唤醒**：点开它，栈回溯会告诉读者它阻塞在哪行、等的是锁还是网络。

除时间线外，工具还提供几张派生视图：goroutine 分析（每个 goroutine 的运行、阻塞、等待时间
拆解）、网络/同步/系统调用阻塞画像、以及调度延迟分布。换句话说，**`pprof` 给出一棵聚合的
调用树，`go tool trace` 给出一部逐帧的运行纪录片**。前者擅长「谁吃 CPU」，后者擅长「时间都
耗在哪段等待上」，二者回答的是正交的问题。
...
**不落盘**，只在判断「出事了」的那一刻，把这段最近历史一次性 dump 出来。Go 1.25 把它从
...
常驻」。让飞行记录仪一直转着，几乎不增加稳态开销，只在告警触发、SLO 越界、或一次慢请求
...
看 [9 调度器](../../part3concurrency/ch09sched)、[13 垃圾回收](../../part4memory/ch13gc) 在读者
